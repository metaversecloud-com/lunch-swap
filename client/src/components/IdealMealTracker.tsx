  ...
  return (
    <section aria-label="Ideal meal tracker">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-lg font-bold text-gray-800">Today's Ideal Meal</h2>
      ...
      >
        <div
          className={`h-full rounded-full transition-all duration-500 ${
            allCollected ? "bg-green-500" : "bg-blue-500"
          }`}
      ...
      {/* 5-slot display */}
      <div
        className={`flex gap-2 ${allCollected ? "motion-safe:animate-pulse" : ""}`}
        role="list"
        ...
        {items.map((item) => {
          ...
          return (
            <div
              key={item.itemId}
              role="listitem"
              className={`relative flex-1 min-w-0 rounded-xl border-2 p-2 text-center transition-all duration-300 ${
                item.collected
                  ? "bg-white shadow-md"
                  : "bg-gray-100 opacity-60"
              } ${
                allCollected
                  ? "motion-safe:shadow-[0_0_12px_rgba(34,197,94,0.5)]"
                  : ""
              }`}
            ...
            >
              ...
              <p
                className={`text-[10px] font-semibold leading-tight truncate ${
                  item.collected ? "text-gray-800" : "text-gray-400"
                }`}
              >
                {item.name}
              </p>
              ...
              <p
                ...
                style={{ color: item.collected ? rarityConfig.color : "#9CA3AF" }}
              >
                {rarityConfig.label}
              </p>
