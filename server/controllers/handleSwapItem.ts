...
export const handleSwapItem = async (req: Request, res: Response) => {
  try {
    ...
    // 1. Fetch the pickup target first (fail fast if gone)
    ...
    // 4. Delete the pickup target from world (race condition guard)
    try {
    ...
    // 5. Drop the old item into world near visitor (B1: use moveTo for position)
    ...
    // 6. Build new bag: remove dropped, add picked up
    ...
    // 8. Update user data (atomic increments)
    ...
    // 9. Calculate XP earned (DROP + PICKUP with rarity multiplier + optional ideal meal bonus)
