...
export const handleGetNearbyItems = async (req: Request, res: Response) => {
  try {
    ...
    // B1: Use visitor.moveTo for position (NOT visitor.position)
    ...
    const proximityRadius = worldData.proximityRadius || 150;

    // B3: Fetch all food items with isPartial to avoid loading data objects
    ...
    for (const asset of allFoodAssets) {
      // B2: Parse uniqueName for metadata (cast needed: SDK types don't expose uniqueName on DroppedAsset class)
